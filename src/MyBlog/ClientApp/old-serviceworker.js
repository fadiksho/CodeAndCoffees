...
self.addEventListener("fetch", function(event) {
  ...
  if (request.url.match(/\/*.(css|js|ttf|eot|woff)/gi)) {
    event.respondWith(
  ...
  if (request.url.match(/\/*.(png|gif|jpeg|svg)/gi)) {
    event.respondWith(
...
self.addEventListener("push", function(event) {
  if (event.data) {
    ...
    event.waitUntil(registration.showNotification(data["Title"], payload));
  }
});
